...
class BotSection extends Component {
    ...
    this.socket.on(SOCKET_PATHS.BOT_RESPONSE, (eventName, response) => {
      ...
      if (response.newRoom === true && eventName === "WATSON") {
        if (response.sessionId) {
          ...
          this.waCreatedTime = new Date().getTime();
        } else {
  ...
  pushWAMessage = (response) => {
    ...
    if (data && Array.isArray(data)) {
      let messages = [...this.state.messages];
      let isSearchResponse = data.findIndex(
        (x) => x.response_type === "search"
      );
      ...
      if (isCustomDemo) {
        let isExitDemo =
          data[data.length - 1].text.toLowerCase().indexOf("demo done") > -1;
        if (isExitDemo) {
          data.pop();
        }
      }
        ...
        if (x.response_type === "search") {
          if (x.results && x.results.length > 0) {
            messages.push({
              user: "WA",
              message: x.header,
              data: x.results.filter(
                (x) => x.title !== "Cookie Policy | Incede"
              ),
              type: "search-result",
            });
          } else {
  ...
  send = () => {
    if (this.state.msg && this.state.msg.length > 0) {
      ...
      this.setState({
        isLoading:
          this.state.selectedDemo === "COVID-19b" &&
  ...
  sendCustomMessage = (msg, shouldAddToMessages) => {
    ...
    if (
      ...
      if (msg && msg.trim() === this.validPin) {
        ...
        messages.push({
          user: "WA",
          message: "Please enter valid pin to continue",
          type: "text",
        });
        this.setState(
          {
    ...
    this.setState(
      {
        ...
        selectedDemo:
          type === "demo" && isDemoUpdate ? demoType : this.state.selectedDemo,
      },
      () => {
        let isAdd = true;
      ...
      if (this.state.isDemo && demoProperty === "Customer Service Demo") {
        ...
        if (this.state.isDemo && data.demoProperty === "COVID-19b") {
          if (this.state.isPinValid) {
            ...
            messages.push({
              user: "WA",
              message: "Please enter pin to continue",
              type: "text",
            });
          }
        } else {
      ...
      this.setState(
        {
          ...
          msg: "",
          isLoading:
            data.demoProperty === "COVID-19b" && this.state.isPinValid === false
              ? false
              : true,
        },
        this.scrollToBottom
  ...
  handleDiscoveryViewContent = (messageIndex, discoveryIndex) => {
    let messages = [...this.state.messages];
    messages[messageIndex].data[discoveryIndex].isExpanded = !messages[
      messageIndex
    ].data[discoveryIndex].isExpanded;
    this.setState({
      messages,
    });
  };
                      ...
                      this.state.isPinValid && (
                        <React.Fragment>
                          ...
                          <div className="d-block d-sm-none">
                            <UncontrolledPopover
                              className="pop-up-container xs"
                              trigger="click"
                              toggle={() => this.toggleLanguagePopUp(true)}
                              isOpen={this.state.isXSLanguagePopUpOpen}
                              placement="top"
                              target="languages_popup_xs"
                            >
                              <PopoverBody className="p-0">
                                <ul className="languages-container xs">
                                  {Object.keys(SUPPORTED_LANGUAGES).map((x) => (
                                    <li
                                      key={x}
                                      onClick={() => this.changeLanguage(x)}
                                      className={`${
                                        this.state.selectedLanguage === x
                                          ? "selected"
                                          : ""
                                      }`}
                                    >
                    ...
                    <React.Fragment>
                      ...
                      <div className="d-none d-sm-block">
                        <UncontrolledPopover
                          ...
                          trigger="click"
                          toggle={() => this.toggleLanguagePopUp(false)}
                          isOpen={this.state.isLanguagePopUpOpen}
                          placement="top"
                          target="languages_popup"
                        >
                          <PopoverBody className="p-0">
